...
public class AppExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<Object> handleAnyException(Exception ex, WebRequest request) {
		...
		ErrorMessage errorMessage = new ErrorMessage(new Date(),
				"Adam able to handle any exception: " + errorDescription);
	...
	public ResponseEntity<Object> dataIntegrityViolation(DataIntegrityViolationException ex, WebRequest request) {
		String errorDescription = ex.getRootCause().getMessage();
		ErrorMessage errorMessage = new ErrorMessage(new Date(), errorDescription);
		return new ResponseEntity<>(errorMessage, new HttpHeaders(), HttpStatus.BAD_REQUEST);
	}

	@ExceptionHandler(value = { SQLIntegrityConstraintViolationException.class })
	public ResponseEntity<Object> handleSqlIntegrityViolation(SQLIntegrityConstraintViolationException ex,
			WebRequest request) {
		String errorDescription = ex.getSQLState();
		ErrorMessage errorMessage = new ErrorMessage(new Date(), errorDescription);
		return new ResponseEntity<>(errorMessage, new HttpHeaders(), HttpStatus.FORBIDDEN);
	}
	@ExceptionHandler(value = { HttpMediaTypeException.class })
	public ResponseEntity<Object> handleHttpMediaTypeNotAcceptable(HttpMediaTypeException ex,
			WebRequest request) {
